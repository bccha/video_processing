...
void generate_color_bar_pattern() {
  ...
  // Force Address Span Extender window to Physical Address 0x20000000
  IOWR_32DIRECT(ADDRESS_SPAN_EXTENDER_0_CNTL_BASE | CACHE_BYPASS_MASK, 0,
                0x20000000);
...
void run_dma_control_submenu() {
  while (1) {
    ...
    if (c == '1') {
      ...
      // Update DMA Frame Pointer
      IOWR_32DIRECT(HDMI_SYNC_GEN_BASE | CACHE_BYPASS_MASK, REG_FRAME_PTR,
                    new_addr);

      // Update Nios Address Span Extender Window
      IOWR_32DIRECT(ADDRESS_SPAN_EXTENDER_0_CNTL_BASE | CACHE_BYPASS_MASK, 0,
                    new_addr);

      printf("Frame Pointer and Span Extender updated to 0x%08X\n", new_addr);
    }
  }
}
...
void run_dma_video_test() {
  ...
  // 2. Set Frame Pointer in RTL (DDR3 Address via Bridge)
  // Force DMA to read from Physical Address 0x20000000 (SDRAM offset)
