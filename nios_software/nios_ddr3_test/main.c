...
void test_ddr_memory() {
  ...
  int *p =
      (int *)(ADDRESS_SPAN_EXTENDER_0_WINDOWED_SLAVE_BASE | CACHE_BYPASS_MASK);
  *p = 0x00;
  printf("before ");
  printf("%p", p);
  printf(" : ");
...
void run_interactive_menu() {
  ...
  while (1) {
    ...
    case 'r':
      IOWR_32DIRECT(
          HDMI_SYNC_BASE | CACHE_BYPASS_MASK, 0,
          0); // Assuming 0 is the pattern register, or REG_PATTERN_MODE
