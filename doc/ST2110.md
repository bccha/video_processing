...
```text
ARM (Linux)                             FPGA Fabric (Top-level Verilog & Qsys)
────────────────                ─────────────────────────────────────────────────────────
 AF_PACKET        LWH2F Bridge
 Raw Socket     (Write Pointer)                [ Qsys (soc_system) ]
     │                │   ──────►┌───────────────┐        ┌───────────────┐ (기존)
     ▼                ▼   │      │ Read mSGDMA   │        │ video_dma     │
DDR Ring Buffer ──────┼───┘      │ (rx_dma_read) │        │ (Read Only)   │
(0x30_000000)         │          │ - MM Read     │        │ - MM Read     │
                      │          │ - ST Source   │        │ - ST Source   │
                      │          └───────┬───────┘        └───────┬───────┘
                      │                  │ 32-bit                 │ 32-bit
                      │                  ▼                        ▼
                      │          [ Top-level Verilog에서 Wire로 연결 (Routing) ]
                      │                  │                        │
                      │          ┌───────▼────────────────┐       │
                      │          │ RTL: Header Stripper   │       │
                      │   8-bit  │ - Drops 62-byte header │       │
                      │   ──ST──►│ - Asserts SOP / EOP    │       │
                      │          └───────┬────────────────┘       │
                      │                  │ 8-bit ST               │
                      │          ┌───────▼────────────────┐       │
                      │          │ RTL: 8-to-32 Alignment │       │
                      │   32-bit │ - Packs RGB(24)+Dummy  │       │
                      │   ◄──ST──┤ - Preserves SOP / EOP  │       │
                      │          └───────┬────────────────┘       │
                      │                  │ 32-bit ST              │
                      │          [ Qsys (soc_system) ]            │
                      │                  ▼                        │
                      ...
                      │          │ Write mSGDMA  │                │
                      │          │ (rx_dma_write)│                │
DDR Frame Buffer ◄────┼──────────┤ - ST Sink     │                │
(0x20_000000)         │          │ - MM Write    │                │
     │                │          └───────────────┘                │
     │                │                                           │
     └────────────────┼───────────────────────────────────────────┘
                      ▼            Interrupt
