...
### 🛡️ 3. 디스플레이의 구원자: 저계조 에너지 누적기 (Low-Gray Energy Accumulator)
바로 이 지점에서 파이프라인의 맨 끝단, **'디더링(Dithering) IP'**가 등판합니다. 이 로직은 고전적인 에러 디퓨전을 하드웨어의 물리적 특성에 맞게 재창조한 것입니다.

1. **에너지의 응축 (Accumulation):** 내 원래 픽셀 값에 주변에서 받은 에러를 더해봐도 Threshold(`0x10`)를 넘지 못하면, 어차피 안 켜질 거 억지로 점등을 시도하지 않고 출력을 `0`으로 꺼버립니다. 대신, 그 미세한 에너지 전체(`sum`)를 고스란히 이웃들에게 전파하며 응축시킵니다.
2. **발화 (Ignition):** 에너지가 구르고 굴러 눈덩이처럼 커지다가 마침내 `0x10`을 넘는 순간, 드라이버가 확실히 인식할 수 있는 전압으로 **빛을 빵! 하고 터뜨립니다(Output = Sum).** 이때 내가 가진 모든 에너지를 빛으로 소화했으므로 옆집으로 줄 에러는 `0`으로 리셋합니다.
3. **결과:** 인간의 눈은 시간적/공간적으로 이 불규칙하게 깜빡이는 점들을 적분하여, **"아주 미세하고 부드러운 검정색의 계조"**가 살아있다고 느끼게 됩니다. 

이 방식은 밝은 영역(Threshold 이상)에서는 에러 전파를 완전히 차단하여 원본의 선명함을 유지하고, 어두운 영역에서만 '에너지 집적'을 통해 블랙 크러시를 해결하는 **지능형 화질 엔진**의 정수입니다. 
